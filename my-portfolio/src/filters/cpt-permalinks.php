...
function custom_project_permalink_structure( $post_link, $post ) {
    if ( post_type_exists( 'projects' ) && 'projects' == $post->post_type ) {
        $terms = get_the_terms( $post->ID, 'category' );
        if ( $terms && ! is_wp_error( $terms ) ) {
            $category = array_shift( $terms );
            $category_slug = $category->slug;
            $post_link = home_url( '/projects/' . $category_slug . '/' . $post->post_name . '/' );
        } else {
            $post_link = home_url( '/projects/' . $post->post_name . '/' );
        }
    }
    return $post_link;
}
