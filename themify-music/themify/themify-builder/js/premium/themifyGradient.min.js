...
k){var c=this;n=a(b);n.parent().children().removeClass("themify_current_point");n.addClass("themify_current_point");var d=n.css("backgroundColor"),m=parseInt(n.css("left")),m=Math.round(m/this.settings.width*100),d=d.substr(4,d.length),d=d.substr(0,d.length-1);e.find(".point-color div").remove();var f=v.find(".themify-color-picker");1>f.length&&(f=a('<input type="text" class="themify-color-picker" />'),f.appendTo(v).minicolors({opacity:!0,change:function(a,b){var d=c.hexToRgb(a);n.css("backgroundColor",
