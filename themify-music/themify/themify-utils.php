...
function themify_is_theme_skin() {
	...
	if ( ! isset( $theme_skin ) ) {
		...
		// Check if $current_skin is not empty before passing it to parse_url
		if ( ! empty( $current_skin ) ) {
			$parsed_skin = parse_url( $current_skin, PHP_URL_PATH );
		} else {
			$parsed_skin = ''; // or handle it differently based on your requirement
		}
		// Check if $parsed_skin is not empty before passing it to dirname
		if ( ! empty( $parsed_skin ) ) {
			$basedir_skin = basename( dirname( $parsed_skin ) );
		} else {
			$basedir_skin = ''; // or handle it differently based on your requirement
		}
...
function themify_get_image( $args ) {
	...
	if ( version_compare( $wp_version, '5.5', '<' ) ) {
		$out = wp_filter_content_tags( $out );
	}
...
function themify_get_theme_names(){
	...
	$themes_list = array(); // Initialize themes_list as an empty array
