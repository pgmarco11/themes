 ...
 * @since 1.6.2
 */
function themify_theme_module_ajax($data=array()){
        /**
	 * Variable key in theme settings
	 * @var string
	 */
	$key = 'setting-disable_ajax';

	/**
	 * Module markup
	 * @var string
	 */
	return  sprintf('<p><span class="label">%1$s</span><label for="%2$s"><input type="checkbox" id="%2$s" name="%2$s" %3$s /> %4$s</label></p><small>%5$s</small>',
		__( 'Ajax Navigation', 'themify' ),
		$key,
		checked( themify_get( $key ), 'on', false ),
		__( 'Disable ajax navigation', 'themify' ),
		__('Ajax navigation allows users to navigate the site without page reload and keep the footer music player continue to play. If you encounter plugin conflicts or navigation issues, try to disable it.','themify')
	);
