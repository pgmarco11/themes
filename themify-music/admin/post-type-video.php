...
if ( ! class_exists( 'Themify_Video' ) ) {
	...
	class Themify_Video {
		...
		function fetch_video_image( $make_tag = true, $width = '' ) {
			...
			if ( $make_tag ) {
				$title = esc_attr(str_replace('"',"'",$title));
				return '<img alt="' . $title . '"   title="' . $title . '" src="' . themify_https_esc( $image_url ) . '" width="' . $width . '" />';
			}
			return $image_url;
		}
		...
		function get_featured_image_link( $args = array() ) {
			...
			$procent = urlencode('100%');
			if ( '' != themify_get( 'video_url' ) ) {
				$link = themify_get( 'video_url' );
				
				$do_iframe = '?iframe=true&width='.$procent.'&height='.$procent;
				$link = esc_url($link . $do_iframe) . '" class="themify_lightbox';
			} elseif ( themify_get( 'external_link' ) != '' ) {
