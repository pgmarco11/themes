	...
	class Themify_Event {
		...
		function meta_boxes_script() {
			...
			if ($this->post_type == $screen->post_type) {
				wp_enqueue_script('themify-theme-custom-panel', themify_enque(THEME_URI . '/js/themify-custom-panel.js'), array('jquery'));
		...
		public function data_update_notice() {
			...
			if ( get_option( 'theme_event_data_update_152_done' ) != 'yes' ) {
		...
		public function do_data_updater() {
			if ( isset( $_GET['theme_event_update_datetime'] ) && true == $_GET['theme_event_update_datetime'] && get_option( 'theme_event_data_update_152_done' ) != 'yes' ) {
				...
				if ( $events ) {
					...
					update_option( 'theme_event_data_update_152_done', 'yes' );
				}
